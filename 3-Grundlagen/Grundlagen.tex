...
Dieses einfache Beispiel nutzt wiederum eine \textit{pure functions} die bereits durch die Programmiersprache, in diesem Fall Scala, bereitgestellt wird. Die Addition zweiter Zahlen durch den \textit{+} Operator. Auch in nicht funktionalen Sprachen, gibt es viele Ausdrücke und \textit{pure functions}, die die \textit{referenctial transparency} waren. Zum Beispiel die mathematischen Operatoren oder die Funktion zur Bestimmung der Länge einer Zeichenkette --- meist \textit{length} genannt.\\
