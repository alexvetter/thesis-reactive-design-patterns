...
Actor-basierte Concurrency oder kurz das \enquote{Actor Modell} stammt ursprünglich vom Carl Hewitt et al und ihrem Artikel \enquote{A universal modular ACTOR formalism for artificial intelligence} aus dem Jahr 1973. Die grundlegende Idee des Actor Modells, ist die Abstraktion der Concurrency durch asynchronen Nachrichtenausstausch zwischen den Komponenten. Eine Komponente in einer Actor-basierten Anwendung wird Actor genannt.
...
Actor-basierte Anwendung setzen somit auf \textit{mutable state} --- jedoch wird diese nicht geteilt. Ein Actor kapselt seinen Zustand und interagiert nur über Nachrichten mit anderen Actors.
