...
Das Observable Pattern stammt aus dem Hause Microsoft und wurde von Erik Meijer erstmalig in dem Framework für reaktive Programming \textit{Reactive Extensions} definiert. Die reaktive Programmierung beschäftigt sich mit der nebenläufigen und asynchronen Verarbeitung von Events. Bei der Entwicklung von Benutzeroberflächen ist reaktive Programmierung seit langem üblich und verbreitet. Eine Benutzeroberfläche muss stetig auf Eingaben, also Events, durch den Benutzer reagieren. Dazu gehören neben Tastatureingaben und Mausklicks auch Cursorbewegungen. Ebenso kann man aber auch Anfragen an eine Webapplikation als stetigen Strom von Events bezeichen.\\
Zu Beginn der Arbeit wurde bereits die Definition reaktiver Systeme genannt. Allgemeiner betrachtet bedeutet das englische Wort \textit{reactive} laut dem Merriam-Webster Wörterbuch \enquote{readily responsive to a stimulus}. Demnach ist etwas \textit{reactive}, wenn es bereitwillig auf einen Reiz reagiert. Im Bezug auf Software sind Reize Events, die von einer Komponente verarbeitet werden müssen \cite{rappl_introduction_2016} \cite[S.~4]{carkci_dataflow_2014} \cite[S.~5]{blackheath_functional_2015}.\\
...
Observable
 .concat(Observable.from(w), Observable.from(m))
...
Das Beispiel zeigt die Verwendung der Reactive Extensions für Java --- kurz RxJava. Es werden zwei Listen mit weiblichen und männlichen Vornamen verbunden (siehe Z.~5). Der daraus resultierende Strom von Vornamen wird nach Namen gefiltert, welche mit \enquote{A} beginnen (siehe Z.~6). Zum Schluss wird ein Subscriber implemententiert, welcher die Namen ausgibt.\\ 
Anstelle der zwei Liste könnte man nun auch Anfragen von einem Sockets abarbeiten. Die einzelnen Funktion oder auch der Subscriber kann wiederverwendet werden.\\

Observables eignen sich asynchrone und nicht blockierenden Daten zu verarbeiten. Zudem eignen sie sich auch zur Verarbeitung von Events bzw. Messages. Für die Entwicklung von message-driven Applikationen sind Bibliotheken, wie Reactive Extensions deshalb sehr nützlich. Jedoch wird keine der vier reaktiven Eigenschaften durch die Verwendung erfüllt.
